...
$adoptionContractValidator = Join-Path $PSScriptRoot 'validate-adoption-contracts.ps1'
...
$componentSchema = Join-Path $outputs '07-capability-component-pattern-adoption.schema.json'
$benchmarkProtocol = Join-Path $outputs '08-empirical-benchmark-protocol.md'
$scorecardSchema = Join-Path $outputs '09-adoption-scorecard.schema.json'
$refactorPlan = Join-Path $outputs '10-abk-pattern-adoption-refactor-plan.md'
$docx = Join-Path $outputs 'sdd-framework-system-design.docx'
foreach ($required in @($design, $patterns, $componentSchema, $benchmarkProtocol, $scorecardSchema, $refactorPlan, $docx)) {
...
$adoptionContractResult = (& $adoptionContractValidator -WorkspaceRoot $workspaceRoot | ConvertFrom-Json)
& (Join-Path $PSScriptRoot 'test-adoption-regressions.ps1') -WorkspaceRoot $workspaceRoot | Out-Null
if ($adoptionContractResult.CompletedCampaigns -ne 0 -or $adoptionContractResult.Chosen -ne 0 -or $adoptionContractResult.Adopted -ne 0) {
    throw "Milestone-zero decision boundary violated: campaigns=$($adoptionContractResult.CompletedCampaigns), CHOSEN=$($adoptionContractResult.Chosen), ADOPTED=$($adoptionContractResult.Adopted)"
}
...
$allowedDispositions = @('runtime_smoked', 'behavior_reproduced', 'covered_by_test', 'static_only_not_executable', 'blocked')
$invalidDispositions = @($allGears | Where-Object { $_.Disposition -notin $allowedDispositions -or [string]::IsNullOrWhiteSpace($_.DispositionEvidence) })
$empiricalClaims = @($allGears | Where-Object { $_.Disposition -in @('runtime_smoked', 'behavior_reproduced', 'covered_by_test') })

if ($allFiles.Count -ne 5027 -or $allGears.Count -ne 5000 -or $pending.Count -ne 0 -or $withoutEvidence.Count -ne 0 -or $invalidDispositions.Count -ne 0) {
    throw "Aggregate coverage mismatch: files=$($allFiles.Count), gears=$($allGears.Count), pending=$($pending.Count), withoutEvidence=$($withoutEvidence.Count), invalidDispositions=$($invalidDispositions.Count)"
}
if ($empiricalClaims.Count -ne 0) {
    throw "Milestone-zero inventory contains $($empiricalClaims.Count) unsupported row-level empirical disposition claims"
}
...
[pscustomobject]@{
    ...
    StaticOnlyNotExecutable = @($allGears | Where-Object Disposition -eq 'static_only_not_executable').Count
    BlockedPendingEmpiricalCampaign = @($allGears | Where-Object Disposition -eq 'blocked').Count
    RowLevelEmpiricalClaims = $empiricalClaims.Count
    PatternCitations = 'valid'
    AdoptionContracts = 'valid: 2 stored positives, 4 mutation negatives, 9 regression rejections, 3 threshold outcomes'
    EmpiricalCampaignExecuted = $adoptionContractResult.EmpiricalCampaignExecuted
    CompletedCampaigns = $adoptionContractResult.CompletedCampaigns
    Chosen = $adoptionContractResult.Chosen
    Adopted = $adoptionContractResult.Adopted
