...
$pilotSchemaPath = Join-Path $workspaceFull 'benchmarks\schemas\abk-native-pilot.schema.json'
...
$manifestPath = Join-Path $campaignRoot 'branches\abk_native\manifest.json'
...
function New-Request([object]$Case, [int]$Repeat, [string]$RunRoot, [object]$Campaign, [string]$RunnerHash, [string]$SchemaHash, [string]$CampaignHash, [string]$SnapshotHash) {
    $fixturePath = Join-Path $campaignRoot ($Case.fixture -replace '/', '\')
    return [ordered]@{
        schema_version = '1.0.0'
        request_id = "abk:run-request:artifact-dag-core-v1-abk-native-$($Case.slug)-r$Repeat"
        ...
        snapshot = [ordered]@{ relative_path = 'benchmarks/snapshots/abk-native-ai-booster-kit-feature.json'; sha256 = $SnapshotHash }
        run = [ordered]@{
            run_id = "abk:run:artifact-dag-core-v1-abk-native-$($Case.slug)-r$Repeat"
        ...
        runner = [ordered]@{ contract_version = 'abk-native-runner-v2'; executable_sha256 = $RunnerHash; host = 'codex' }
    }
}

function Assert-CaseResult([object]$Case, [object]$Run, [object]$Oracle, [object]$Provenance) {
    if ($Run.branch -ne 'abk_native' -or $Run.branch_id -ne 'abk_native') { throw "RUN_BRANCH_INVALID: $($Case.case_id)" }
    ...
    if ($null -eq $Case.error_code) { if ($null -ne $Run.error) { throw "RUN_UNEXPECTED_ERROR: $($Case.case_id)" } }
    elseif ($null -eq $Run.error -or $Run.error.code -ne $Case.error_code) { throw "RUN_ERROR_MISMATCH: $($Case.case_id) expected $($Case.error_code)" }
    ...
    if ($Provenance.external_project_read -ne $false -or $Provenance.source_code_copied_into_framework_matrix -ne $false -or $Provenance.git_linked_into_framework_matrix -ne $false) { throw "PROVENANCE_SCOPE_INVALID: $($Case.case_id)" }
    if ([string]::IsNullOrWhiteSpace([string]$Provenance.snapshot_revision)) { throw "PROVENANCE_REVISION_MISSING: $($Case.case_id)" }
}

$campaign = Read-Json $campaignPath
if (-not (Test-Json -LiteralPath $campaignPath -SchemaFile $schemaPath)) { throw 'CAMPAIGN_SCHEMA_INVALID: campaign does not satisfy benchmark-campaign.schema.json' }
...
if ($manifest.status -ne 'READY_FOR_EXECUTION' -or $manifest.disposition.candidate_disposition -ne 'RUNNABLE') { throw 'MANIFEST_NOT_RUNNABLE: abk_native manifest must be READY_FOR_EXECUTION/RUNNABLE' }
if ($manifest.entrypoint_sha256 -ne $runnerHash -or $manifest.snapshot_sha256 -ne $snapshotHash) { throw 'MANIFEST_HASH_MISMATCH: manifest pins are stale' }
...
foreach ($case in $cases) {
    $runRoot = Join-Path $outputFull ("$($case.case_id)\abk_native\R1")
    New-Item -ItemType Directory -Path $runRoot -Force | Out-Null
    $requestPath = Join-Path $runRoot 'request.json'
    Write-Json (New-Request $case 1 $runRoot $campaign $runnerHash $schemaHash $campaignHash $snapshotHash) $requestPath
    ...
    if (-not (Test-Path -LiteralPath $runPath -PathType Leaf) -or -not (Test-Path -LiteralPath $oraclePath -PathType Leaf) -or -not (Test-Path -LiteralPath $provenancePath -PathType Leaf)) { throw "RUN_EVIDENCE_MISSING: $($case.case_id)" }
    $run = Read-Json $runPath; $oracle = Read-Json $oraclePath; $provenance = Read-Json $provenancePath
    if ($exitCode -ne $case.exit_code) { throw "RUN_PROCESS_EXIT_MISMATCH: $($case.case_id) expected $($case.exit_code), got $exitCode" }
    Assert-CaseResult $case $run $oracle $provenance
    $evidenceId = "abk-native-pilot-$($case.slug)-r1"
    $runEvidenceIds.Add($evidenceId)
    $results.Add([ordered]@{
        evidence_id = $evidenceId; case_id = $case.case_id; status = 'passed'; expected_terminal_state = $case.terminal_state; terminal_state = $run.terminal_state; exit_code = $run.exit_code; oracle_status = $oracle.status; error_code = if ($null -ne $run.error) { $run.error.code } else { $null }; comparable = [bool]$provenance.comparable
        run_relative_path = ([IO.Path]::GetRelativePath($workspaceFull, $runPath)).Replace('\', '/'); run_sha256 = Get-Sha256 $runPath; oracle_relative_path = ([IO.Path]::GetRelativePath($workspaceFull, $oraclePath)).Replace('\', '/'); oracle_sha256 = Get-Sha256 $oraclePath
    })
}

$capturedAt = [DateTime]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ssZ')
$pilot = [ordered]@{
    '$schema' = '../../../../schemas/abk-native-pilot.schema.json'; schema_version = '1.0.0'; pilot_id = 'abk:pilot:abk-native-bounded-v1'; campaign_id = $campaign.campaign_id; branch = 'abk_native'; captured_at = $capturedAt; status = 'passed'; full_campaign = $false; raw_runs = @($results).Count; expected_full_campaign_runs = $campaign.run_policy.expected_raw_runs; campaign_sha256 = $campaignHash; schema_sha256 = $schemaHash; snapshot_sha256 = $snapshotHash; manifest_sha256 = $manifestHash; entrypoint_sha256 = $runnerHash; pilot_schema_sha256 = $pilotSchemaHash; results = @($results); run_evidence_ids = @($runEvidenceIds); scorecard_state = 'UNSCORED'; blockers = @('ABK-native is a local clean-room mapping, not execution of the external AI Booster Kit project.', 'The published campaign remains benchmark_pending with zero completed runs and no adoption outcome.')
}
$pilotPath = Join-Path $outputFull 'pilot.json'
Write-Json $pilot $pilotPath
if (-not (Test-Json -LiteralPath $pilotPath -SchemaFile $pilotSchemaPath)) { throw 'PILOT_SCHEMA_INVALID: bounded pilot ledger does not satisfy abk-native-pilot.schema.json' }
$markdown = [System.Collections.Generic.List[string]]::new()
$markdown.Add('# ABK-native bounded pilot'); $markdown.Add(''); $markdown.Add("- pilot_id: $($pilot.pilot_id)"); $markdown.Add("- captured_at: $capturedAt"); $markdown.Add('- status: passed'); $markdown.Add('- scope: one run for each of the ten frozen fixtures; abk_native only'); $markdown.Add('- full_campaign: false'); $markdown.Add("- raw_runs: $($pilot.raw_runs) / $($pilot.expected_full_campaign_runs) expected for the complete three-branch campaign"); $markdown.Add('- scorecard: UNSCORED; no adoption outcome is asserted'); $markdown.Add('- external_project_execution: false; immutable snapshot used as metadata-only provenance'); $markdown.Add(''); $markdown.Add('| Case | Terminal | Exit | Oracle | Comparable | Evidence |'); $markdown.Add('|---|---|---:|---|---|---|'); foreach ($result in $results) { $markdown.Add("| $($result.case_id) | $($result.terminal_state) | $($result.exit_code) | $($result.oracle_status) | $($result.comparable) | $($result.evidence_id) |") }
[IO.File]::WriteAllText((Join-Path $outputFull 'pilot.md'), ($markdown -join "`n"), $utf8NoBom)
Write-Output "ABK_NATIVE_BOUNDED_COMPLETE: $($pilot.raw_runs)/$($pilot.raw_runs) PASS; output=$outputFull"
